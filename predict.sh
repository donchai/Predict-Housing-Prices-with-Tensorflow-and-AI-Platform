...
gcloud compute ssh --project ${PROJECTID} --zone ${ZONE} ${PROJECTID} --quiet -- -v 'source /tmp/remotevar.sh && cd /home/jupyter/training-data-analyst/blogs/housing_prices && gcloud ai-platform jobs submit training $JOBNAME --region=$REGION --module-name=trainer.task --package-path=./trainer --job-dir=$GCS_BUCKET/$JOBNAME/ --runtime-version 1.15 -- --output_dir=$GCS_BUCKET/$JOBNAME/output &'

gcloud compute ssh --project ${PROJECTID} --zone ${ZONE} ${PROJECTID} --quiet -- -v 'source /tmp/remotevar.sh && cd /home/jupyter/training-data-analyst/blogs/housing_prices && gcloud ai-platform jobs submit training $JOBNAME --region=$REGION --module-name=trainer.task --package-path=./trainer --job-dir=$GCS_BUCKET/$JOBNAME --runtime-version 1.15 --scale-tier=STANDARD_1 -- --output_dir=$GCS_BUCKET/$JOBNAME/output &'

gcloud compute ssh --project ${PROJECTID} --zone ${ZONE} ${PROJECTID} --quiet -- -v 'source /tmp/remotevar.sh && cd /home/jupyter/training-data-analyst/blogs/housing_prices && gcloud ai-platform jobs submit training $JOBNAME --region=$REGION --module-name=trainer.task --package-path=./trainer --job-dir=$GCS_BUCKET/$JOBNAME --runtime-version 1.15 --scale-tier=BASIC_GPU -- --output_dir=$GCS_BUCKET/$JOBNAME/output &'

gcloud compute ssh --project ${PROJECTID} --zone ${ZONE} ${PROJECTID} --quiet -- -v 'source /tmp/remotevar.sh && cd /home/jupyter/training-data-analyst/blogs/housing_prices && gcloud ai-platform jobs submit training $JOBNAME --region=$REGION --module-name=trainer.task --package-path=./trainer --job-dir=$GCS_BUCKET/$JOBNAME --runtime-version 1.15 --config config.yaml -- --output_dir=$GCS_BUCKET/$JOBNAME/output &'

gcloud compute ssh --project ${PROJECTID} --zone ${ZONE} ${PROJECTID} --quiet -- -v 'source /tmp/remotevar.sh && source /tmp/remotemod.sh && cd /home/jupyter/training-data-analyst/blogs/housing_prices && gcloud ai-platform models create ${MODEL_NAME} --regions $REGION --quiet && gcloud ai-platform versions create ${MODEL_VERSION} --model ${MODEL_NAME} --origin ${MODEL_LOCATION} --staging-bucket=$GCS_BUCKET --runtime-version=1.15 --quiet && gcloud ai-platform predict --model housing_prices --json-instances records.json --quiet'
